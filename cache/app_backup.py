...
def app(request,fun):
    ...
    if fun=='app_login':
        ...
        if(new_value['fichier_audio']):
            ...
            if new_value['fichier_audio']:
                if 'yzzhenli-1256427631' not in str(new_value['fichier_audio']):
            ...
            if new_value['visuel']:
                if 'yzzhenli-1256427631' not in str(new_value['visuel']):
        ...
        for un in values:
            if 'yzzhenli-1256427631' not in str(un['visuel']) and un['visuel']:
                un['visuel'] = 'https://www.yzzhenli.org' + str(un['visuel'])
            if 'yzzhenli-1256427631' not in str(un['fichier_audio']) and un['fichier_audio']:
        ...
        data = request.GET
        res=models.pray.objects.create(contenu=data['contenu'],user_id=data['user_id'],user_name=data['user_name'])
        if res:
            value='suc'
        else:
            value = 'fail'
        ...
        for un in values:
            if un["fichier_audio"]:
                if 'yzzhenli-1256427631' not in str(un['fichier_audio']):
                    un["fichier_audio"] = host + un["fichier_audio"]
            if un["visuel"]:
                if 'yzzhenli-1256427631' not in str(un['visuel']):
                    un["visuel"] = host + un["visuel"]
        values = list(values)
        return HttpResponse(json.dumps(values, cls=DateEncoder))
    elif fun=='un_cours':
...
def apps(request,fun):
    ...
    if fun=='app_login':
        ...
        value = models.article.objects.filter(id=data['id']).values()
        res = value[0]
        if 'yzzhenli-1256427631' not in str(res['fichier_audio']) and res['fichier_audio']:
        ...
        if data["type"]=="article":
            ...
            for un in cathegorie:
                if un["visuel"]:
                    if "https://yzzhenli-1256427631.cos" not in str(un["visuel"]):
                        un["visuel"]="https://www.yzzhenli.org"+str(un["visuel"])
                else:
        ...
        limit = int(data['limit'])
        n = int(data['n'])
        values=models.pray.objects.order_by('-id').exclude(user_id=3467)[n*limit:(n+1)*limit].values()
        values = list(values)
        return HttpResponse(json.dumps(values, cls=DateEncoder))
    elif fun=='beni_priere':
        data = request.POST
